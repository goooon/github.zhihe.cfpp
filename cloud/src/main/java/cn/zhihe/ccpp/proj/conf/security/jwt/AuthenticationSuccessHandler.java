...
import cn.zhihe.ccpp.func.cst.SecurityConstant;
...
public class AuthenticationSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
    ...
    @Value("${cfpp.tokenExpireTime}")
    private Integer tokenExpireTime;

    @Value("${cfpp.saveLoginTime}")
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        if(StrUtil.isNotBlank(saveTime) &&
                Boolean.valueOf(saveTime)){
        ...
        ResponseUtil.out(response, ResponseUtil.resultMap(true,200, ResponseString.LOGIN_SUCCESS, token));
    }
}
